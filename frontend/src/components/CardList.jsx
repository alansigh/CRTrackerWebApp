...
import { Archive, Filter, ArrowUpDown, X, Loader2 } from "lucide-react"
...
function CardList() {
  ...
  const renderCard = (card, mode = 'normal') => {
    ...
    const iconUrl = getCardIcon(card, mode);

    return (
      <div
        key={`${mode}-${card.id || card.name}`}
        onClick={() => handleCardClick(card, mode)}
        className={`relative bg-[#181822] rounded-2xl border-2 p-2 flex flex-col items-center justify-between shadow-skeuo-outset group hover:-translate-y-2 hover:scale-[1.05] transition-all duration-300 cursor-pointer ${rarityClasses}`}
      >
        <div className="absolute -top-12 -left-12 z-30 flex items-center justify-center w-32 h-32">
            ...
            <div className="flex flex-wrap gap-2 p-1 bg-obsidian rounded-xl shadow-skeuo-inset border border-slate-light/30">
              {['all', 'common', 'rare', 'epic', 'legendary', 'champion'].map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-lg font-mono text-xs uppercase transition-all duration-300 flex-1 md:flex-none ${filter === f
            ...
            <div className="flex flex-wrap gap-2 p-1 bg-obsidian rounded-xl shadow-skeuo-inset border border-slate-light/30">
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-mono text-xs uppercase transition-all duration-300 ${elixirSort === s.id
            ...
            <div className="flex justify-between items-center p-6 border-b border-white/10 bg-obsidian">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center bg-slate-900 border ${getRarityColorClass(selectedCard.rarity).split(' ')[1]}`}>
                  <img
                    src={getCardIcon(selectedCard, selectedCardMode)}
